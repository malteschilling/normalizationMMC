"""
A MMC network for a three segmented manipulator.
...
class mmcArmNetwork:
    ...
    def compute_normalization_step(self):
        ...
        self.l_1_norm = np.squeeze(self.normalization_model.predict(np.expand_dims(self.l_1, axis=0))) #self.l_1/np.linalg.norm(self.l_1)
        self.l_2_norm = np.squeeze(self.normalization_model.predict(np.expand_dims(self.l_2, axis=0))) #self.l_2/np.linalg.norm(self.l_2)
        self.l_3_norm = np.squeeze(self.normalization_model.predict(np.expand_dims(self.l_3, axis=0))) #self.l_3/np.linalg.norm(self.l_3)
        #print(self.l_1_norm - self.l_1/np.linalg.norm(self.l_1))
    
    """ Normalization """
    def compute_linalg_normalization_step(self):
        self.l_1_norm = self.l_1/np.linalg.norm(self.l_1)
        self.l_2_norm = self.l_2/np.linalg.norm(self.l_2)
        self.l_3_norm = self.l_3/np.linalg.norm(self.l_3)
        #print(self.l_1_norm - self.l_1/np.linalg.norm(self.l_1))
    ...
    def update_variables(self):
        self.l_1_hat = self.l_1
        self.l_2_hat = self.l_2
        self.l_3_hat = self.l_3
    ...
    def draw_manipulator(self):
        ...
        #plt.savefig("/Users/mschilling/Desktop/Swing_Top_"+str(step)+".pdf")
